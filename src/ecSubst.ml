...
type subst = {
  ...
  sb_flocal  : form Mid.t;
  sb_fmem    : EcIdent.t Mid.t;
  sb_tydef   : (EcIdent.t list * ty) Mp.t;
  sb_def     : (EcIdent.t list * [`Expr of expr | `Form of form]) Mp.t;
}
...
let subst_path (s : subst) (p : EcPath.path) =
  EcPath.p_subst s.sb_path p

(* -------------------------------------------------------------------- *)
let subst_mpath (s : subst) (mp : EcPath.mpath) =
  EcPath.m_subst (subst_path s) s.sb_module mp

(* -------------------------------------------------------------------- *)
let subst_xpath (s : subst) (xp : EcPath.xpath) =
  EcPath.x_substm (subst_path s) s.sb_module xp
...
let add_tyvar (s : subst) (x : EcIdent.t) (ty : ty) =
  ...
  { s with sb_tyvar = Mid.add x ty s.sb_tyvar }
...
let add_flocal (s : subst) (x : EcIdent.t) (f : form) =
  { s with sb_flocal = Mid.add x f s.sb_flocal }

let add_flocals (s : subst) (xs : EcIdent.t list) (fs : form list) =
...
and subst_modtype (s : subst) (modty : module_type) =
  { mt_params = List.map (snd_map (subst_modtype s)) modty.mt_params;
    mt_name   = subst_path s modty.mt_name;
...
and fresh_glocal (s : subst) ((x, ty) : EcIdent.t * gty) =
  let xfresh = EcIdent.fresh x in
  let ty = subst_gty s ty in
  let s =
    match ty with
    | GTty ty ->
       add_flocal s x (f_local xfresh ty)

    | GTmem _ ->
       add_memory s x xfresh

    | GTmodty _ ->
       add_module s x (EcPath.mident xfresh) in
